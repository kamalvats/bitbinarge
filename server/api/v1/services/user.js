...
const userServices = {
      ...
      },
      {
        $match: {
          $or: [
            ...
            { mobileNumber: { $regex: data, $options: "i" } }
          ]
        }
      },
      {
    ...
    searchData.push(
      {
        $project: {
          ...
          // 'subscriptionDetails.tradeFee':'$subscriptionDetails.tradeFee',
          speakeasy: 1,
          referredByData: 1,
  ...
  findUserWithPopulate: async (query) => {
    return await userModel.findOne(query).populate('referredBy');
  },
  findUserCount: async (query) => {
    return await userModel.countDocuments(query);
  },
  findAllUserWithSelectedField: async (query) => {
    return await userModel.find(query).select('firstName lastName userName email mobileNumber userType status ibiId ibiName _id');
  },
}
