        ...
        try {
            ...
            if (!userResult) {
                ...
                if (userResult.userType != userType.ADMIN) {
                    ...
                    // }
                    if (!(checkSubscriptionPlan.arbitrageName).includes(arbitrage.DirectArbitrage) || !(checkSubscriptionPlan.exchangeUID).includes((req.body.buy).toLowerCase())) {
                ...
                if (directdata) {
                    ...
                    if (directdata.strategy[1].isTradeActive == true && directdata.strategy[1].isTrade == false) {
                        ...
                        if (cancelData2.status == true) {
                            ...
                            let orderObj1 = { userId: userResult._id, body: cancelData2.error, title: `Quantum Flow Trade Cancellation - Order ${directdata.strategy[1].orderId}`, orderId: directdata.strategy[1].orderId, }
                            let notification = await notificationCreate(orderObj1);
                            if (notification) {
                                await notificationCreate(orderObj1);
                            }
                            throw apiError.badRequest("Direct Quantum Flow sell order Not cancelled.");
                        }
                    }
                        ...
                        if (cancelData1.status == true) {
                            ...
                            let orderObj1 = { userId: userResult._id, body: cancelData1.error, title: `Quantum Flow Trade Cancellation - Order ${directdata.strategy[0].orderId}` }
                            let notification = await notificationCreate(orderObj1);
                            if (notification) {
                                await notificationCreate(orderObj1);
                            }
                            throw apiError.badRequest("Direct Quantum Flow buy order Not cancelled.");
                        }
                    }
