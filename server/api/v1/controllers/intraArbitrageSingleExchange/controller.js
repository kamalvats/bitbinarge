...
const { intraArbitrageSingleExchangeCreate, intraArbitrageSingleExchangeData, intraArbitrageSingleExchangeList, intraArbitrageSingleExchangeUpdate, intraArbitrageSingleExchangeArbitrageListWithPaginate, intraArbitrageSingleExchangeArbitrageListWithPaginate1, intraArbitrageSingleExchangeAllList, intraAbitrageSingleExchangeDelete } = intraAbitrageSingleExchangeServices;
...
export class intraAbritrageSingleExchangeController {
    ...
    async filterProfitPaths(req, res, next) {
        try {
            ...
            if (!userResult) {
                ...
                let isWhitelistCountries = false
                if (userResult.country) {
                    let checkCountries = whitelistCountries.find((items) => { return items.code == userResult.country })
                    if (checkCountries) {
                        isWhitelistCountries = true
                    }
                }
                        ...
                        if (req['body']['exchangeUID'] && req['body']['exchangeUID'].length != 0) {
                            for (let exchange of connectedExchangeDetails) {
                                if (req['body']['exchangeUID'].includes(exchange['uid'])) {
                                    ...
                                    if (!accountBalances) {
                                        throw apiError.badRequest(responseMessage.TRY_AGAIN);
                                    }
        ...
        try {
            ...
            if (!userResult) {
                ...
                if (userResult.userType != userType.ADMIN) {
                    ...
                    if (!(checkSubscriptionPlan.arbitrageName).includes(arbitrage.IntraArbitrageSingleExchange) || !(checkSubscriptionPlan.exchangeUID).includes((req.body.exchangeName).toLowerCase())) {
                    ...
                    if (checkSubscriptionPlan.isFuelDeduction == true) {
                        if (checkSubscriptionPlan.coinType == 'USD') {
                            if (Number(userResult.fuelUSDBalance) <= 0.5) {
                                isTrue = true
                            }
                        }
    ...
    */
    ...
    /**
    * @swagger
    * /intraArbitrageSingleExchange/allListPlacedTradeWithFilter:
    *   post:
    *     tags:
    *       - INTRA ARBITRAGE SINGLE
    *     description: List of Intra single exchange place trade
    ...
    */
    async allListPlacedTradeWithFilter(req, res, next) {
        ...
        try {
            ...
            let intraSingleExchangeArbitrageList = await intraArbitrageSingleExchangeArbitrageListWithPaginate1(validatedBody);
            if (intraSingleExchangeArbitrageList.docs.length == 0) {
                throw apiError.notFound(responseMessage.DATA_NOT_FOUND);
            } else {
                return res.json(new response(intraSingleExchangeArbitrageList, responseMessage.DATA_FOUND));
    ...
    /**
    * @swagger
    * /intraArbitrageSingleExchange/script2:
    *   put:
    *     tags:
    *       - INTRA ARBITRAGE SINGLE
    *     description: List of INTRA ARBITRAGE SINGLE profit paths
    ...
    */
    async script2(req, res, next) {
        try {
            ...
            let query = { arbitrageStatus: "FAILED", status: { $ne: "DELETE" } }
            let intraRes = await intraArbitrageSingleExchangeAllList(query);
            for (let i = 0; i < intraRes.length; i++) {
                if (intraRes[i].strategy[0].orderId == "" && intraRes[i].strategy[0].status == "") {
                    let result = await intraAbitrageSingleExchangeDelete({ _id: intraRes[i]._id })
                }
            }
    ...
    /**
    * @swagger
    * /intraArbitrageSingleExchange/script13:
    *   get:
    *     tags:
    *       - INTRA ARBITRAGE SINGLE
    *     description: Get sniper bot data
    ...
    */
    async script13(req, res, next) {
        try {
            ...
            if (!userResult) {
                ...
                let sniperBotDetails = await sniperData({ userId: userResult._id, arbitrageName: 'IntraSingleExchange' });
                if (sniperBotDetails) {
                    return res.json(new response(sniperBotDetails, responseMessage.DATA_FOUND));
                } else {
                    throw apiError.notFound(responseMessage.DATA_NOT_FOUND);
                }
            }
    ...
    /**
    * @swagger
    * /intraArbitrageSingleExchange/script14:
    *   get:
    *     tags:
    *       - INTRA ARBITRAGE SINGLE
    *     description: List of intra arbitrage single profit paths
    ...
    async script14(req, res, next) {
        try {
            ...
            if (!userResult) {
                ...
                let autoTradeDetails = await autoTradeData({ userId: userResult._id, arbitrageName: 'IntraSingleExchange' });
                if (autoTradeDetails) {
                    return res.json(new response(autoTradeDetails, responseMessage.DATA_FOUND));
