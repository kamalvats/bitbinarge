...
import whitelistCountries from '../../../../helper/whitelistCountries.json'
            ...
            for (let user of users) {
                ...
                if (user.userType != userType.ADMIN) {
                    ...
                    if (checkSubscriptionPlan) {
                        ...
                        if (checkSubscriptionPlan.isFuelDeduction == true) {
                            if (checkSubscriptionPlan.coinType == 'USD') {
                                if (Number(user.fuelUSDBalance) <= 0.5) {
                                    isTrue = true
                                }
                            }
                    ...
                    planExchange = checkSubscriptionPlan.exchangeUID
                } else {
                    planExchange = ['binance', 'kraken', 'mexc', 'bitmart', 'coinbase']
                }
                ...
                if (user.connectedExchange.length != 0) {
                    ...
                    for (let connectedData of connectedExchangeDetails) {
                        if (autoTradeDetails['exchangeUID'].includes(connectedData['uid'])) {
                            ...
                            if ((planExchange).includes((exchangeName).toLowerCase())) {
                                ...
                                let usdtBalance = accountBalances.filter(function (entry) { return (entry.asset === "USDT" || entry.asset === "USDC"); });
                                if (usdtBalance.length != 0) {
                                    usdtBalance = usdtBalance.filter(items => { return items.total > Number(autoTradeDetails.capital) })
                                    let startCurrency = usdtBalance.reduce((acc, item) => {
                                        if (item.total > Number(autoTradeDetails.capital)) {
                                            acc.push(item.asset);
                                        }
                                        return acc;
                                    }, []);
                                    if (startCurrency.length != 0) {
                                        ...
                                        let allProfitPath = await triangularProfitPaths(exchangeName, '', '', '', '', '3', Number(autoTradeDetails.capital))
                                        ...
                                        if (!totalProfitPaths || totalProfitPaths.length == 0) {
                                            ...
                                            if (user.country) {
                                                let checkCountries = whitelistCountries.find((items) => { return items.code == user.country })
                                                if (checkCountries) {
                                                    totalProfitPaths = totalProfitPaths.filter(items => { return items.which_pair != 'USDT' })
                                                }
                                            }
                                            ...
                                            if (totalProfitPaths.length != 0) {
                                                ...
                                                for (let b = 0; b < usdtBalance.length; b++) {
                                                    totalProfitPaths = totalProfitPaths.filter(function (el) {
                                                    ...
                                                    if (totalProfitPaths != 0) {
                                                        ...
                                                        let query = { status: "ACTIVE", arbitrageStatus: "PENDING", arbitrageType: arbitrageType.AUTO }
                                                        let pendingRes = await triangularModel.findOne(query)
                                                        if (!pendingRes) {
                                        ...
                                        let lowBalanceObj = {
                                            userId: user._id,
                                            title: `Low Balance Alert on ${exchangeName} for Quantum loop`,
                                            body: `Action Required: Quantum loop Disabled Due to Insufficient Capital ($${autoTradeDetails.capital}) on ${exchangeName}`
                                        }
                        ...
                        if (autoTradeDetails['exchange1'].includes(connectedData['uid'])) {
                            ...
                            if ((planExchange).includes((exchangeName).toLowerCase())) {
                                ...
                                if (usdtBalance.length != 0) {
                                    ...
                                    if (startCurrency.length != 0) {
                                        if (autoTradeDetails.exchange2 != 0 && autoTradeDetails.capital) {
                                            var pathdata = await profitPathModel.findOne({ arbitrageName: arbitrage.DirectArbitrage })
                                            if (pathdata) {
                                                let exchangeArray = [connectedData['uid']]
                                                for (let b = 0; b < usdtBalance.length; b++) {
                                                    let filterProfitData = pathdata.path.filter(p => p.pair == usdtBalance[b]['asset'])
                                                    data = await arbitragefunction.getExchange(filterProfitData, exchangeArray, autoTradeDetails.exchange2);
                                                    ...
                                                    if (data.length != 0) {
                                                        ...
                                                        if (data.length != 0) {
                                                            if (user.country) {
                                                                let checkCountries = whitelistCountries.find((items) => { return items.code == user.country })
                                                                if (checkCountries) {
                                                                    data = data.filter(items => { return items.which_pair != 'USDT' })
                                                                }
                                                            }
                                                            ...
                                                            if (data.length != 0) {
                                                                ...
                                                                if (connected1 && connected2) {
                                                                    ...
                                                                    let query = { status: "ACTIVE", arbitrageStatus: "PENDING", arbitrageType: arbitrageType.AUTO }
                                                                    let pendingRes = await directModel.findOne(query)
                                                                    if (!pendingRes) {
                                        ...
                                        let lowBalanceObj = {
                                            userId: user._id,
                                            // title: `Low Balance Alert on ${exchangeWithlowBalance[0]} for Quantum flow`,
                                            title: `Low Balance Alert for Quantum flow`,
                                            // body: `Action Required: Quantum flow Disabled Due to Insufficient Capital ($${autoTradeDetails.capital}) on ${exchangeWithlowBalance[0]}`
                                            body: `Action Required: Quantum flow Disabled Due to Insufficient Capital ($${autoTradeDetails.capital})`
                                        }
                                        let notificationDetails = await notificationData(lowBalanceObj);
                                        if (notificationDetails) {
                                            lowBalanceObj['isRead'] = false;
                                        ...
                                        if (lowBalancearray.length == autoTradeDetails['exchange1'].length) {
                                            let updateUser = await userModel.findByIdAndUpdate({ _id: user._id }, { $set: { 'autoTrade.direct': false, 'autoTradePlaceCount.direct': 0 } }, { new: true });
                                            await commonFunction.sendEmailInsufficientBalance(user)
                                        }
                                    }
                            ...
                            if ((planExchange).includes((exchangeName).toLowerCase())) {
                                ...
                                if (usdtBalance.length != 0) {
                                    ...
                                    if (startCurrency.length != 0) {
                                        ...
                                        if (!totalProfitPaths || totalProfitPaths.length == 0) {
                                            ...
                                            if (totalProfitPaths.length != 0) {
                                                ...
                                                if (totalProfitPaths != 0) {
                                                    ...
                                                    console.log('65 ===>', highestProfitPath);
                                                    let query = { status: "ACTIVE", arbitrageStatus: "PENDING", arbitrageType: arbitrageType.AUTO }
                                                    let pendingRes = await intraSingleExchange.findOne(query)
                                                    if (!pendingRes) {
                                        ...
                                        let lowBalanceObj = {
                                            userId: user._id,
                                            title: `Low Balance Alert on ${exchangeName} for Quantum bridge`,
                                            body: `Action Required: Quantum bridge Disabled Due to Insufficient Capital ($${autoTradeDetails.capital}) on ${exchangeName}`
                                        }
                                        let notificationDetails = await notificationData(lowBalanceObj);
                                        if (notificationDetails) {
                                            lowBalanceObj['isRead'] = false;
