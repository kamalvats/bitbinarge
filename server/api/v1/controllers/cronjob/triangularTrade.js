...
const { createFuelWalletDeductionHistory, findFuelWalletDeductionHistory } = fuelWalletDeductionHistoryServices
...
let triangularSniperTime1 = Date.now() + minutes * 60 * 1000
...
let executeTriangularArbitrage = new CronJob(config.get('cronTime.triangularArbitrage'), async () => {
    try {
        ...
        for (let a = 0; a < triangularArbitrageList.length; a++) {
            console.log("MANUAL==================================>>>>>>>>>>>>>>>>>", a, triangularArbitrageList[a]._id)
            ...
            let [connectedExchangeData, tickers] = await Promise.all([connectedExchangeModel.findOne({ _id: obj.connectedExchangeId }), exchangeModel.findOne({ exchangeName: exchange })])
            ...
            if (connectedExchangeData) {
                ...
                for (let i = 0; i < obj.strategy.length; i++) {
                    ...
                    let tradingPairs = symbol
                    if (tickers[symbol]) {
                        tradingPairs = tickers[symbol].symbol
                    }
                    if (obj.strategy[i].status == 'cancelled' || obj.strategy[i].status == 'ORDER_NOT_FOUND') {
                        ...
                        if (arbitrageStatusUpdate) {
                            ...
                            if (checkNotificationOnOff) {
                                let notificationObj = {
                                    ...
                                    symbol: symbol,
                                    title: `Quantum Loop Trade Cancellation - Order ${obj.strategy[i].orderId}`,
                                    body: `Your Quantum Loop trade (Order ${obj.strategy[i].orderId}) for ${symbol} has been cancelled.`
                                }
                        ...
                        if (obj.start != "USDT" && obj.start != "USDC") {
                            if (obj.strategy[i].action == 'buy' && (obj.strategy[i].quoteCurrency === "USDT" || obj.strategy[i].quoteCurrency === "USDC")) {
                                profit = parseFloat(obj.filledTotal) - parseFloat(obj.capitalInUSDT)
                            }
                            if (obj.strategy[i].action == 'sell' && (obj.strategy[i].quoteCurrency === "ETH" || obj.strategy[i].quoteCurrency === "BTC")) {
                                if (tickers[obj.start + "USDT"]) {
                                    profit = parseFloat(profit) * parseFloat(tickers[obj.start + "USDT"].price)
                                }
                            }
                        }
                        ...
                        if (arbitrageStatusUpdate) {
                            ...
                            if (checkNotificationOnOff) {
                                let notificationObj = {
                                    ...
                                    orderId: obj.strategy[i].orderId,
                                    symbol: symbol,
                                    title: `Quantum Loop Trade Successful - Order ${obj.strategy[i].orderId}`,
                                    body: `Congratulations! Your Quantum Loop trade (Order ${obj.strategy[i].orderId}) for ${symbol} has been successfully executed!`
                                }
                                await notificationCreate(notificationObj)
                            }
                            await commonFunction.sendEmailCloseTrade(updateUser.email, "Hi", "Quantum Loop", symbol, obj.capital, profit)
                        }
                        break;
                        ...
                        if (obj.strategy[i].isTrade == false && obj.strategy[i].isActive == false) {
                            ...
                            token = [obj.strategy[i].baseCurrency, obj.strategy[i].quoteCurrency];
                            if (action == 'sell') {
                                token = [obj.strategy[i].quoteCurrency, obj.strategy[i].baseCurrency];
                            }
                            if (action == 'buy') {
                                if (i == 0) {//HAI_ETH
                                    ...
                                    if (Object.values(obj.strategy[actionValue])[0] == 'buy') {
                                        ...
                                        quantity = filledTotal / finalPrice;
                                    }
                                    // quantity = Object.values(obj.strategy[i - 1])[6] / finalPrice
                                    // quantity = amount / finalPrice
                                }
                                orderPlaced = await buy(exchange, symbol, tradingPairs, price, quantity, apiKey, secretKey, passphrase, customerId, api_memo);
                                ...
                                orderPlaced = await sell(exchange, symbol, tradingPairs, finalPrice, quantity, apiKey, secretKey, passphrase, customerId, api_memo);
                                if (orderPlaced) {
                            ...
                            if (orderPlaced) {
                                if (orderPlaced && orderPlaced.isTradable == true) {
                                    ...
                                    let [arbitrageStatusUpdate, checkNotificationOnOff] = await Promise.all([
                                        triangularModel.findByIdAndUpdate({ _id: obj._id }, { status: status.BLOCK, arbitrageStatus: "FAILED" }, { new: true }),
                                        userModel.findOne({ _id: obj.userId, 'notifications.trade_error': true })
                                    ])
                                    if (arbitrageStatusUpdate) {
                                        if (arbitrageStatusUpdate.arbitrageType == arbitrageType.AUTO) {
                                            let updateUser = await userModel.findByIdAndUpdate({ _id: obj.userId }, { $set: { 'autoTradePlaceCount.triangular': 0 } }, { new: true });
                                    ...
                                    if (checkNotificationOnOff) {
                                        let notificationObj = {
                                            userId: obj.userId,
                                            symbol: symbol,
                                            title: `Quantum Loop Trade Unsuccessful - Symbol ${symbol}`,
                                            body: `Regrettably, your Quantum Loop trade (Symbol ${symbol}) on ${exchange} has encountered a failure.`
                                        }
                                        await notificationCreate(notificationObj)
                                    }
                                }
                            ...
                            let [filteredOrderData, triangularData] = await Promise.all([
                                filter_triangularOrderDetails(exchange, orderData, symbol, tickers),
                                triangularModel.findOne({ _id: placedArbitrageId })
                            ])
                            ...
                            if (newPathArray.length != 0) {
                                try {
                                    if (newPathArray[i].status == 'closed') {
                                        ...
                                        let amount = filteredOrderData.orderAmount;
                                        let filledTotal = filteredOrderData.orderFilledTotal;
                                        let [updateOrderRes, updateStrategy] = await Promise.all([
                                            orderModel.findOneAndUpdate({ orderId: newPathArray[i].orderId }, { $set: { orderStatus: newPathArray[i].status, executedQty: amount, cummulativeQuoteQty: filledTotal } }, { new: true }),
                                            triangularModel.findByIdAndUpdate({ _id: placedArbitrageId }, { $set: { strategy: newPathArray } }, { new: true })
                                        ])
                                    }
                                } catch (error) {
...
let executeAutoTriangularArbitrage = new CronJob(config.get('cronTime.triangularArbitrage'), async () => {
    try {
        ...
        // }
        for (let a = 0; a < triangularArbitrageList.length; a++) {
            console.log("AUTO==================================>>>>>>>>>>>>>>>>>", a, triangularArbitrageList[a]._id)
            obj = triangularArbitrageList[a];
            let exchange = obj.exchangeName;
            // let [connectedExchangeData, tickers] = await Promise.all([connectedExchangeModel.findOne({ _id: obj.connectedExchangeId, status: status.ACTIVE }), exchangeModel.findOne({ exchangeName: exchange })])
            let [connectedExchangeData, tickers] = await Promise.all([connectedExchangeModel.findOne({ _id: obj.connectedExchangeId }), exchangeModel.findOne({ exchangeName: exchange })])
                ...
                for (let i = 0; i < obj.strategy.length; i++) {
                    ...
                    if (obj.strategy[i].status == 'cancelled' || obj.strategy[i].status == 'ORDER_NOT_FOUND') {
                        ...
                        if (arbitrageStatusUpdate) {
                            ...
                            await commonFunction.sendEmailCloseTrade(updateUser.email, "User", "Quantum Loop", symbol, obj.capital, profit)
                        }
                        break;
                    } else if (obj.strategy[i].status == 'rebalancing' && obj.strategy[i].isActive == true) {
                        console.log("========================>>>>>453")
                        ...
                        if (obj.strategy[i].isTrade == false && obj.strategy[i].isActive == false) {
                            ...
                            if (action == 'buy') {
                                ...
                                // console.log('591 sell ==>', exchange, symbol, finalPrice, quantity, apiKey, secretKey, passphrase, customerId)
                                orderPlaced = await sell(exchange, symbol, tradingPairs, finalPrice, quantity, apiKey, secretKey, passphrase, customerId, api_memo);
                                if (orderPlaced) {
                                    orderPlaced.side = 'sell'
                                }
...
let executeSniperTriangularArbitrage = new CronJob(config.get('cronTime.triangularArbitrage'), async () => {
    try {
        ...
        let result = await executeSniperTriangularArbitrageFunction("SNIPER0")
...
let executeSniperTriangularArbitrage1 = new CronJob(config.get('cronTime.triangularArbitrage'), async () => {
    try {
        executeSniperTriangularArbitrage1.stop()
        let result = await executeSniperTriangularArbitrageFunction("SNIPER1")
        triangularSniperTime1 = Date.now() + minutes * 60 * 1000
        executeSniperTriangularArbitrage1.start()
    } catch (error) {
        executeSniperTriangularArbitrage1.start()
    }
});
...
let executeAllTriangularArbitrage = new CronJob(config.get('cronTime.triangularArbitrage'), async () => {
    try {
        ...
        // }
        for (let a = 0; a < triangularArbitrageList.length; a++) {
            console.log("ALL==================================>>>>>>>>>>>>>>>>>", a, triangularArbitrageList[a]._id)
            obj = triangularArbitrageList[a];
            let exchange = obj.exchangeName;
            // let [connectedExchangeData, tickers] = await Promise.all([connectedExchangeModel.findOne({ _id: obj.connectedExchangeId, status: status.ACTIVE }), exchangeModel.findOne({ exchangeName: exchange })])
            let [connectedExchangeData, tickers] = await Promise.all([connectedExchangeModel.findOne({ _id: obj.connectedExchangeId }), exchangeModel.findOne({ exchangeName: exchange })])
...
async function fuelDeduction(userId, profit, objId) {
    let [userRes, subscriptionPlan] = await Promise.all([
        userModel.findOne({ _id: userId }),
        lastedBuyPlan({ userId: userId })
    ])
    if (0 < Number(profit)) {
        if (subscriptionPlan && userRes) {
            if (subscriptionPlan.isFuelDeduction == true) {
                let amount = (Number(profit) * Number(subscriptionPlan.tradeFee)) / 100
                let check = await findFuelWalletDeductionHistory({ triangularId: objId })
                if (!check) {
                    if (subscriptionPlan.coinType == 'USD') {
                        await Promise.all([
                            userModel.findByIdAndUpdate({ _id: userId }, { $inc: { fuelUSDBalance: -Number(amount) } }, { new: true }),
...
let allCronRestart = new CronJob(config.get('cronTime.allCronReset'), async () => {
    ...
    if (triangularSniperTime1 < Date.now()) {
        executeSniperTriangularArbitrage1.start()
    }
...
async function executeSniperTriangularArbitrageFunction(sniperGroup) {
    try {
        let query = { sniperGroup: sniperGroup, status: "ACTIVE", arbitrageStatus: "PENDING", isFirstStrategy: true, arbitrageType: arbitrageType.SNIPER }
        let triangularArbitrageList = await triangularModel.find(query);
        let count = 0;
        let obj
        console.log("Total triangularArbitrage ==>sniper", sniperGroup, triangularArbitrageList.length)
        for (let a = 0; a < triangularArbitrageList.length; a++) {
            console.log("SNIPER==================================>>>>>>>>>>>>>>>>>", sniperGroup, a, triangularArbitrageList[a]._id)
            obj = triangularArbitrageList[a];
            let exchange = obj.exchangeName;
            // let [connectedExchangeData, tickers] = await Promise.all([connectedExchangeModel.findOne({ _id: obj.connectedExchangeId, status: status.ACTIVE }), exchangeModel.findOne({ exchangeName: exchange })])
            let [connectedExchangeData, tickers] = await Promise.all([connectedExchangeModel.findOne({ _id: obj.connectedExchangeId }), exchangeModel.findOne({ exchangeName: exchange })])
                    ...
                    if (obj.strategy[i].status == 'cancelled' || obj.strategy[i].status == 'ORDER_NOT_FOUND') {
                        ...
                        if (obj.strategy[i].isTrade == false && obj.strategy[i].isActive == false) {
                            ...
                            if (action == 'buy') {
                                ...
                                orderPlaced = await buy(exchange, symbol, tradingPairs, price, quantity, apiKey, secretKey, passphrase, customerId, api_memo);
                                // console.log('Buy orderPlaced 894 ==>', orderPlaced)
                                if (orderPlaced) {
                                    orderPlaced.side = 'buy'
                                }
                            }
