                            ...
                            if ((planExchange).includes((exchangeName).toLowerCase())) {
                                ...
                                if (usdtBalance.length != 0) {
                                    ...
                                    if (startCurrency.length != 0) {
                                        ...
                                        let lowBalanceObj = {
                                            userId: user._id,
                                            title: `Low Balance Alert on ${exchangeName} for Quantum flow`,
                                            body: `Action Required: Quantum flow Disabled Due to Insufficient Capital ($${sniperBotDetails.capital}) on ${exchangeName}`
                                        }
                        ...
                        for (let connectedData of connectedExchangeDetails) {
                            if (sniperBotDetails['exchangeUID'].includes(connectedData['uid'])) {
                                ...
                                if ((planExchange).includes((exchangeName).toLowerCase())) {
                                    ...
                                    if (usdtBalance.length != 0) {
                                        usdtBalance = usdtBalance.filter(items => { return items.total > Number(sniperBotDetails.capital) })
                                        let startCurrency = usdtBalance.reduce((acc, item) => {
                                            if (item.total > Number(sniperBotDetails.capital)) {
                                                acc.push(item.asset);
                                            }
                                        ...
                                        if (startCurrency.length != 0) {
                                                ...
                                                if (totalProfitPaths.length != 0) {
                                                    ...
                                                    if (totalProfitPaths != 0) {
                                                        ...
                                                        for (let b = 0; b < usdtBalance.length; b++) {
                                                            ...
                                                            if (filterProfitPath.length != 0) {
                                                                let query = { userId: user._id, status: "ACTIVE", arbitrageStatus: "PENDING", arbitrageType: arbitrageType.SNIPER }
                                                                ...
                                                                if (finalTrade.length != 0) {
                                                                    ...
                                                                    for (let z = 0; z < finalLoopNumber; z++) {
                                                                        ...
                                                                        let array = ["SNIPER1", "SNIPER0"];
                                                                        let randomElement = array[Math.floor(Math.random() * array.length)];
                                                                        highestProfitPath.sniperGroup = randomElement
                                            ...
                                            let lowBalanceObj = {
                                                ...
                                                body: `Action Required: Quantum loop Disabled Due to Insufficient Capital ($${sniperBotDetails.capital}) on ${exchangeName}`
                                            }
                                            let notificationDetails = await notificationData(lowBalanceObj);
                                            if (notificationDetails) {
                                                lowBalanceObj['isRead'] = false;
