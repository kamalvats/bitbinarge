                            ...
                            if (newPathArray.length != 0) {
                                try {
                                    if (newPathArray[i].status == 'closed') {
                                        ...
                                        let updateStrategy = await intraArbitrageSingleExchangeUpdate({ _id: placedArbitrageId }, { $set: { strategy: newPathArray, amount: amount, filledTotal: filledTotal,isFirstStrategy: false } }, { new: true });
    ...
    if (0 < Number(profit)) {
        if (subscriptionPlan && userRes) {
            if (subscriptionPlan.isFuelDeduction == true) {
                ...
                let check = await findFuelWalletDeductionHistory({ intraId: objId })
                if (!check) {
