                                ...
                                if (orderDetail.status == true) {
                                    ...
                                    if (checkNotificationOnOff) {
                                        let notificationObj = {
                                            ...
                                            title: `Quantum Flow Trade Cancelled - Order ${obj.strategy[i].orderId}`,
                                            body: `The Quantum Flow trade associated with Order ${obj.strategy[i].orderId} for ${obj.base + obj.pair} has been cancelled.`
                                        }
                            ...
                            if (orderPlace.isTradable == true) {
                                ...
                                if (checkNotificationOnOff) {
                                    let notificationObj = {
                                        userId: userId,
                                        symbol: symbol,
                                        title: `Quantum Flow Trade Failed - Symbol ${symbol}`,
                                        body: `Your Quantum Flow trade (Symbol ${symbol}) has unfortunately failed on ${exchange}.`
                                    }
                                    await notificationCreate(notificationObj)
                                }
                                ...
                                if (withdrow && withdrow.iswithdrawable == true) {
                                    ...
                                    if (checkNotificationOnOff) {
                                        let notificationObj = {
                                            userId: userId,
                                            symbol: obj.base,
                                            title: `Quantum Flow Withdraw Failed - Symbol ${obj.base}`,
                                            body: `Regrettably, your Quantum Flow withdrawal (Symbol ${obj.base}) has encountered an unfortunate failure on ${obj.strategy[i].exchange}.`
                                        }
                                        await notificationCreate(notificationObj)
                                    }
                                    let updateStrategy = await directModel.findByIdAndUpdate({ _id: placeddirecteId }, { $set: { status: 'BLOCK', arbitrageStatus: 'FAILED' } }, { new: true });
                            ...
                            if (orderDetail && orderDetail.status == true) {
                                ...
                                if (updateStrategy) {
                                    ...
                                    if (checkNotificationOnOff) {
                                        let notificationObj = {
                                            ...
                                            orderId: obj.strategy[i].orderId,
                                            symbol: obj.base + obj.pair,
                                            title: `Quantum Flow Trade Executed Successfully - Order ${obj.strategy[i].orderId}.`,
                                            body: `Congratulations! Your Quantum Flow trade (Order ${obj.strategy[i].orderId}) for ${obj.base} and ${obj.pair} has been successfully executed!`
                                        }
                                        await notificationCreate(notificationObj)
                                    }
                                ...
                                if (checkNotificationOnOff) {
                                    let notificationObj = {
                                        ...
                                        orderId: obj.strategy[i].orderId,
                                        symbol: obj.base + obj.pair,
                                        title: `Quantum Flow Trade Cancellation - Order ${obj.strategy[i].orderId}.`,
                                        body: `The Quantum Flow trade (Order ${obj.strategy[i].orderId}) for ${obj.base + obj.pair} has been cancelled.`
                                    }
                                    await notificationCreate(notificationObj)
                                }
                            ...
                            if (orderPlace.isTradable == true) {
                                ...
                                if (checkNotificationOnOff) {
                                    let notificationObj = {
                                        userId: userId,
                                        symbol: symbol,
                                        title: `Quantum Flow Trade Unsuccessful - Symbol ${symbol}`,
                                        body: `Regrettably, your Quantum Flow trade (Symbol ${symbol}) encountered an unfortunate failure on ${exchange}.`
                                    }
                                    await notificationCreate(notificationObj)
                                }
                                let updateStrategy = await directModel.findByIdAndUpdate({ _id: placeddirecteId }, { $set: { status: "BLOCK", arbitrageStatus: 'FAILED' } }, { new: true });
    ...
    if (0 < Number(profit)) {
        if (subscriptionPlan && userRes) {
            if (subscriptionPlan.isFuelDeduction == true) {
                ...
                let check = await findFuelWalletDeductionHistory({ directId: objId })
                if (!check) {
                    if (subscriptionPlan.coinType == 'USD') {
                        await Promise.all([
                            userModel.findByIdAndUpdate({ _id: userId }, { $inc: { fuelUSDBalance: -Number(amount) } }, { new: true }),
                            createFuelWalletDeductionHistory({ directId: objId, transactionType: "DIRECT", userId: userId, profit: profit, amount: amount, coinName: 'USD', subscriptionId: subscriptionPlan._id, arbitrageName: "Quantum Flow" })
                        ])
                    }
                    if (subscriptionPlan.coinType == 'FIERO') {
                        await Promise.all([
                            userModel.findByIdAndUpdate({ _id: userId }, { $inc: { fuelFIEROBalance: -Number(amount) } }, { new: true }),
                            createFuelWalletDeductionHistory({ directId: objId, transactionType: "DIRECT", userId: userId, profit: profit, amount: amount, coinName: 'FIERO', subscriptionId: subscriptionPlan._id, arbitrageName: "Quantum Flow" })
                        ])
                    }
                }
            }
