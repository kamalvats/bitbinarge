...
import tokenPair from "../../../../helper/tokenpair";
...
import { asks_bids } from '../../../../helper/asks_bids_function'
import { getPair, withdraw_delisted, checkPairs } from '../../../../helper/get_tickers'
...
export class walletController {
            ...
            if (!userResult) {
                ...
                if (Object.keys(req.body).length == 0) {
                    ...
                    if (responseResult.length != 0) {
                        ...
                        return res.json(new response({ allExchangeTotal, responseResult }, responseMessage.EXCHANGE_DETAILS));
                    } else {
                        return res.json(new response([], responseMessage.NOT_CONNECTED));
                    }
                }
    ...
    async orderDetails(req, res, next) {
        try {
            ...
            if (!userResult) {
                ...
                let responseResult;
                let connectedExchangeDetails = await connectedExchangeData({ _id: req.body._id });
                let exchangeDetails = await exchangeData({ uid: connectedExchangeDetails.uid, status: "ACTIVE" });
    ...
    async Dashboard(req, res, next) {
        try {
            ...
            if (totalTriangularProfitRes.length != 0) {
                totalTriangularProfit = totalTriangularProfitRes.reduce((n, { profit }) => n + Number(profit), 0)
            }
    ...
    async DashboardRecentData(req, res, next) {
        try {
            ...
            if (totaltriangular30dayprofit.length != 0) {
                finalTotalTriangular30DayProfit = totaltriangular30dayprofit.reduce((n, { profit }) => n + Number(profit), 0)
            ...
            if (totaltriangular60dayprofit.length != 0) {
                finalTotalTriangular60DayProfit = totaltriangular60dayprofit.reduce((n, { profit }) => n + Number(profit), 0)
            ...
            if (totaltriangular90dayprofit.length != 0) {
                finalTotalTriangular90DayProfit = totaltriangular90dayprofit.reduce((n, { profit }) => n + Number(profit), 0)
    ...
    async cryptoAssetprofit(req, res, next) {
        try {
            ...
            if (btcProfitRes.length != 0) {
                obj1.profit = btcProfitRes.reduce((n, { profit }) => n + Number(profit), 0)
            }
            if (ethProfitRes.length != 0) {
                obj2.profit = ethProfitRes.reduce((n, { profit }) => n + Number(profit), 0)
            }
            if (usdtProfitRes.length != 0) {
                obj3.profit = usdtProfitRes.reduce((n, { profit }) => n + Number(profit), 0)
            }
            ...
            if (btc30ProfitRes.length != 0) {
                obj31.profit = btc30ProfitRes.reduce((n, { profit }) => n + Number(profit), 0)
            }
            if (eth30ProfitRes.length != 0) {
                obj32.profit = eth30ProfitRes.reduce((n, { profit }) => n + Number(profit), 0)
            }
            if (usdt30ProfitRes.length != 0) {
                obj33.profit = usdt30ProfitRes.reduce((n, { profit }) => n + Number(profit), 0)
            }
            ...
            if (btc60ProfitRes.length != 0) {
                obj61.profit = btc60ProfitRes.reduce((n, { profit }) => n + Number(profit), 0)
            }
            if (eth60ProfitRes.length != 0) {
                obj62.profit = eth60ProfitRes.reduce((n, { profit }) => n + Number(profit), 0)
            }
            if (usdt60ProfitRes.length != 0) {
                obj63.profit = usdt60ProfitRes.reduce((n, { profit }) => n + Number(profit), 0)
            }
            ...
            if (btc90ProfitRes.length != 0) {
                obj91.profit = btc90ProfitRes.reduce((n, { profit }) => n + Number(profit), 0)
            }
            if (eth90ProfitRes.length != 0) {
                obj92.profit = eth90ProfitRes.reduce((n, { profit }) => n + Number(profit), 0)
            }
            if (usdt90ProfitRes.length != 0) {
                obj93.profit = usdt90ProfitRes.reduce((n, { profit }) => n + Number(profit), 0)
            }
    ...
    */
    ...
    async getPairs(req, res, next) {
        try {
            let getPairsRes = await findPairs({ status: status.ACTIVE })
            let obj = {
                staticPairs: currencyPairs.currencyPairs,
                pairs: getPairsRes.pairs
            }
            return res.json(new response(obj, responseMessage.DATA_FOUND));
        } catch (error) {
            return next(error);
     ...
     *         required: true
     *       - name: updatepairs
     *         description: updatepairs
     *         in: body
     *         required: true
     *         schema:
     *           $ref: '#/definitions/updatepairs'
     *     responses:
     *       200:
     *         description: Returns success message
     */
    async updatePairs(req, res, next) {
        try {
            ...
            let totalPairs = currencyPairs.currencyPairs
            if (getPairsRes) {
                totalPairs = currencyPairs.currencyPairs.concat(getPairsRes.pairs)
                allDynamicPairs = getPairsRes.pairs
            }
            for (let i = 0; i < req.body.pairs.length; i++) {
                if (!(totalPairs).includes((req.body.pairs[i]).toUpperCase())) {
                    allDynamicPairs.push(req.body.pairs[i])
                }
            }
            let finalArray = []
            allDynamicPairs.map((o) => {
                if (req.body.pairs.includes(o)) {
                    finalArray.push(o);
                }
            })
            finalArray = [...new Set(finalArray)];
            if (finalArray.length > 20) {
                throw apiError.badRequest(responseMessage.ONLY_20_PAIRS_UPDATE);
            }
            let updateRes = await updatePairs({ _id: getPairsRes._id }, { pairs: finalArray })
    ...
    async checkPairs(req, res, next) {
        ...
        try {
            ...
            let validateBody = await Joi.validate(req.query, validationSchema)
            let exchangeList = await exchangeModel.find({ status: status.ACTIVE })
            for (let i = 0; i < exchangeList.length; i++) {
                let pairs = await checkPairs(exchangeList[i].exchangeName, validateBody.coin)
                if (pairs.availbale == true) {
                    obj.availbale = true
                    break;
                }
            }
    ...
    /**
     * @swagger
     * /wallet/exchangeTradeCount:
     *   get:
     *     tags:
     *       - WALLET
     *     description: exchangeTradeCount
     *     produces:
     *       - application/json
     *     parameters:
     *       - name: token
     ...
     *         description: Returns success message
     */
    async exchangeTradeCount(req, res, next) {
        try {
            let userResult = await findUser({ _id: req.userId, userType: { $in: [userType.USER, userType.SUBADMIN, userType.ADMIN] } });
            if (!userResult) {
                throw apiError.notFound(responseMessage.USER_NOT_FOUND);
            ...
            let finalResult = []
            let allExchangeRes = await exchangeList({ status: "ACTIVE" });
            for (let i = 0; i < allExchangeRes.length; i++) {
                let query = { userId: userResult._id, exchangeName: allExchangeRes[i].exchangeName, status: { $ne: status.DELETE } }
                let [triangularCount, intraCount] = await Promise.all([
                    triangularArbitrageCount(query),
                    intraArbitrageSingleExchangeCount(query),

                ])
                let obj = {
                    exchangeName: allExchangeRes[i].exchangeName,
                    totalTrade: triangularCount + intraCount,
                }
    ...
    /**
     * @swagger
     * /wallet/transactionStatistic:
     *   get:
     *     tags:
     *       - WALLET
     *     description: get transactionStatistic
     *     produces:
     *       - application/json
     *     parameters:
     *       - name: token
     ...
     *         description: Returns success message
     */
    async transactionStatistic(req, res, next) {
        try {
            let userResult = await findUser({ _id: req.userId, userType: { $in: [userType.USER, userType.ADMIN, userType.SUBADMIN] } });
            if (!userResult) {
                throw apiError.notFound(responseMessage.USER_NOT_FOUND);
            ...
            if (req.query.type == 'day') {
                var yesterday = new Date(new Date().getTime() - (24 * 60 * 60 * 1000));
                let profitTotalQuery = { $and: [{ createdAt: { $gte: yesterday } }, { createdAt: { $lte: currentDay } }], arbitrageStatus: arbitrageStatus.COMPLETED, userId: userResult._id, profit: { $gte: 0 } }
                const [
                    totalDirectProfitRes,
                    totalTriangularProfitRes,
                    totalIntraArbitrageSingleExchageProfitRes] = await Promise.all([
                        directArbitrageList(profitTotalQuery),
                        triangularAllList(profitTotalQuery),
                        intraArbitrageSingleExchangeAllList(profitTotalQuery),
                    ])
                let totalDirectProfit = 0, totalDirectCapital = 0, totalLoopProfit = 0, totalTriangularProfit = 0, totalTriangularCapital = 0, totalIntraArbitrageSingleExchangeProfit = 0, totalIntraArbitrageSingleExchangeCapital = 0
                if (totalDirectProfitRes.length != 0) {
                    totalDirectProfit = totalDirectProfitRes.reduce((n, { profit }) => n + Number(profit), 0)
                    totalDirectCapital = totalDirectProfitRes.reduce((n, { capitalInUSDT }) => n + Number(capitalInUSDT), 0)
                }
                ...
                if (totalTriangularProfitRes.length != 0) {
                    totalTriangularProfit = totalTriangularProfitRes.reduce((n, { profit }) => n + Number(profit), 0)
                    totalTriangularCapital = totalTriangularProfitRes.reduce((n, { capitalInUSDT }) => n + Number(capitalInUSDT), 0)
                }
                if (totalIntraArbitrageSingleExchageProfitRes.length != 0) {
                    totalIntraArbitrageSingleExchangeProfit = totalIntraArbitrageSingleExchageProfitRes.reduce((n, { profit }) => n + Number(profit), 0)
                    totalIntraArbitrageSingleExchangeCapital = totalIntraArbitrageSingleExchageProfitRes.reduce((n, { capitalInUSDT }) => n + Number(capitalInUSDT), 0)
                }

                let result = [{
                    totalProfit: totalDirectProfit + totalTriangularProfit + totalIntraArbitrageSingleExchangeProfit,
                    totalCapital: totalDirectCapital + totalTriangularCapital + totalIntraArbitrageSingleExchangeCapital,
                    date: new Date()
                }]
                return res.json(new response(result, responseMessage.DATA_FOUND));
            } else if (req.query.type == 'month' || req.query.type == 'week') {
                ...
                for (let i = 0; i < daysOfWeek.length; i++) {
                    let startTime = new Date(new Date(daysOfWeek[i]).toISOString().slice(0, 10))
                    let lastTime = new Date(new Date(daysOfWeek[i]).toISOString().slice(0, 10) + 'T23:59:59.999Z');
                    let profitTotalQuery = { $and: [{ createdAt: { $gte: startTime } }, { createdAt: { $lte: lastTime } }], arbitrageStatus: arbitrageStatus.COMPLETED, userId: userResult._id, profit: { $gte: 0 } }
                    const [
                        totalDirectProfitRes,
                        totalTriangularProfitRes,
                        totalIntraArbitrageSingleExchageProfitRes] = await Promise.all([
    ...
    // async totalProfitAndTotalTradeAndVolumeSocket(req, res, next) {
    //     try {
    //         let responses;
    //         return new Promise(async (resolve, reject) => {
    ...
    //             }
    //             responses = ({ responseCode: 200, responseMessage: "Data found successfully.", responseResult: obj });
    //             resolve(responses)
    //         })
    //     } catch (error) {
    //         console.log("totalProfitAndTotalTrade", error)
    //         return next(error);
    //     }
    // }
    async totalProfitAndTotalTradeAndVolumeSocket(req, res, next) {
        try {
            let responses;
            return new Promise(async (resolve, reject) => {
                let datas = await findTotalProfitAndTotalTradeAndVolumeContent({})
                if (!datas) {
                    responses = ({ responseCode: 404, responseMessage: "Data not found.", responseResult: [] });
                    resolve(responses)
                } else {
                    let obj = {
                        totalProfit: datas.totalProfit,
                        totalTrades: datas.totalTrades,
                        totalTradingVolume: datas.totalTradingVolume
                    }
                    responses = ({ responseCode: 200, responseMessage: "Data found successfully.", responseResult: obj });
                    resolve(responses)
                }
    ...
    /**
    * @swagger
    * /wallet/profitStatistic:
    *   get:
    *     tags:
    *       - WALLET
    *     description: get profitStatistic
    *     produces:
    *       - application/json
    *     parameters:
    *       - name: type
    ...
    *         description: Returns success message
    */
    async profitStatistic(req, res, next) {
        try {
            var currentDay = new Date();
            let weekDataRes = []
            var daysOfWeek = [];
            ...
            if (req.query.type == 'day') {
                ...
                var yesterday = new Date(new Date().getTime() - (24 * 60 * 60 * 1000));
                let dateQuery = { $and: [{ createdAt: { $gte: new Date(yesterday) } }, { createdAt: { $lte: new Date(currentDay) } }] }
                profitTotalQuery = { ...profitTotalQuery, ...dateQuery }
                let [triangularTotalProfit, directTotalProfit, intraSingleExchangeTotalProfit] = await Promise.all([
                    triangularAllList(profitTotalQuery),
                    directArbitrageList(profitTotalQuery),
                ...
                for (let i = 0; i < daysOfWeek.length; i++) {
                    ...
                    let startTime = new Date(new Date(daysOfWeek[i]).toISOString().slice(0, 10))
                    let lastTime = new Date(new Date(daysOfWeek[i]).toISOString().slice(0, 10) + 'T23:59:59.999Z');
                    let dateQuery = { $and: [{ createdAt: { $gte: new Date(startTime) } }, { createdAt: { $lte: new Date(lastTime) } }] }
                    profitTotalQuery = { ...profitTotalQuery, ...dateQuery }
                    let [triangularTotalProfit, directTotalProfit, intraSingleExchangeTotalProfit] = await Promise.all([
                        triangularAllList(profitTotalQuery),
                        directArbitrageList(profitTotalQuery),
    ...
    // async profitStatisticWithSocket(type) {
    //     try {
    //         let responses;
    //         return new Promise(async (resolve, reject) => {
    //             var currentDay = new Date();
    ...
    //                     },
    //                 }
    //                 responses = ({ responseCode: 200, responseMessage: responseMessage.DATA_FOUND, responseResult: obj });
    //                 resolve(responses)
    ...
    //                     weekDataRes.push(objDb);
    //                 }
    //                 responses = ({ responseCode: 200, responseMessage: responseMessage.DATA_FOUND, responseResult: weekDataRes });
    //                 resolve(responses)
    //             } else {
    //                 for (let i = 0; i < 12; i++) {
    //                     let totalDirectProfit = 0, totalTriangularProfit = 0, totalIntraArbitrageSingleExchangeProfit = 0
    ...
    //                 }
    //                 yearDataRes.sort((a, b) => a.year - b.year || a.month - b.month);
    //                 responses = ({ responseCode: 200, responseMessage: responseMessage.DATA_FOUND, responseResult: yearDataRes });
    //                 resolve(responses)
    //             }
    //         })
    ...
    async profitStatisticWithSocket(type) {
        try {
            let responses;
            return new Promise(async (resolve, reject) => {
                let datas = await findprofitStatisticContent({})
                if (!datas) {
                    responses = ({ responseCode: 404, responseMessage: "Data not found.", responseResult: [] });
                    resolve(responses)
                } else {
                    if (type == 'day') {
                        ...
                        responses = ({ responseCode: 200, responseMessage: responseMessage.DATA_FOUND, responseResult: datas.weekly });
                        resolve(responses)
                    } else if (type == 'month') {
                        responses = ({ responseCode: 200, responseMessage: responseMessage.DATA_FOUND, responseResult: datas.monthly });
                        resolve(responses)
                    } else {
                        responses = ({ responseCode: 200, responseMessage: responseMessage.DATA_FOUND, responseResult: datas.yearly });
                        resolve(responses)
                    }
                }
