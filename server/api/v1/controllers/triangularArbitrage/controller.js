...
const { triangularCreate, triangularData, triangularList, triangularUpdate, triangularArbitrageListWithPaginate, triangularArbitrageListWithPaginate1, triangularAllList, triangularDelete } = triangularServices;
...
export class triangularController {
                        ...
                        if (trade.isActive == true && trade.isTrade == false) {
                            ...
                            if (canceldata.status == true) {
                                ...
                                let orderObj = {
                                    ...
                                    title: `Quantum Loop Trade Cancellation - Order ${trade.orderId}`,
                                    orderId: trade.orderId,
                                }
                                let notification = await notificationCreate(orderObj);
                                throw apiError.notFound("Placed Quantum Loop order not cancelled.");
                            }
                        }
    ...
    */
    ...
    /**
    * @swagger
    * /triangularArbitrage/allListPlacedTradeWithFilter:
    *   post:
    *     tags:
    *       - TRIANGULAR ARBITRAGE
    *     description: List of triangular place trade
    ...
    */
    async allListPlacedTradeWithFilter(req, res, next) {
        ...
        try {
            ...
            let triangularArbitrageList = await triangularArbitrageListWithPaginate1(validatedBody);
            if (triangularArbitrageList.docs.length == 0) {
                throw apiError.notFound(responseMessage.DATA_FOUND);
            } else {
                return res.json(new response(triangularArbitrageList, responseMessage.DATA_NOT_FOUND));
            }
        } catch (error) {
            return next(error);
        }
    ...
    */
    async script2(req, res, next) {
        try {
            ...
            for (let i = 0; i < triangularDetails.length; i++) {
                if (triangularDetails[i].strategy[0].orderId == "" && triangularDetails[i].strategy[0].status == "") {
                    let result = await triangularDelete({ _id: triangularDetails[i]._id })
                }
            }
