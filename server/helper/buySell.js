    ...
    switch (exchange) {
      ...
      case 'Coinbase':
        try {
          ...
          console.log('1226 ==> deposit address ', error, error.response.data, new Date().toLocaleString())
      ...
      case 'Bitmart':
        try {
          ...
          console.log('1025 ==>', error, new Date().toLocaleString());
          return { status: false }
        }
        break;
    }
    ...
    switch (exchange) {
      ...
      case 'Mexc':
        try {
          ...
          const serverTime = time.data.serverTime;
          function generate_signature(serverTime) {
            let message = `coin=${base}&address=${address}&amount=${capital}&netWork=${coinNetwork}&timestamp=` + serverTime;
            message = message.replace(/\(/g, '%28').replace(/\)/g, '%29');
            return crypto.createHmac('sha256', secretKey).update(message).digest('hex');
          }
          let sec = generate_signature(serverTime)
    ...
    switch (exchange) {
      ...
      case 'Mexc':
        try {
          ...
          console.log('743 WithdrawalHistory error==>', error, new Date().toLocaleString());
        }
        break;
      case 'Bitmart':
        try {
          ...
          console.log('1458 WithdrawalHistory error==>', error, new Date().toLocaleString());
        }
        break;
    }
  },
    ...
    switch (exchange) {
      ...
      case 'Mexc':
        try {
          ...
          console.log("Mexc deposit", error, new Date().toLocaleString())
        }
        break;
      case 'Bitmart':
        try {
          ...
          console.log('1676 bitmart deposit history error==>', error, new Date().toLocaleString());
        }
        break;
    }
  },
    ...
    switch (exchange) {
      ...
      case 'Bitmart':
        try {
          ...
          console.log('1824 WithdrawalHistory error==>', error, new Date().toLocaleString());
          return { iswithdrawable: false };
        }
        break;
    }
    ...
    switch (exchange) {
      ...
      case 'Coinbase':
        try {
          ...
          console.log('1100 ==>withdraw Crypto  history', error, new Date().toLocaleString())
        }
        break;
      case 'Kraken':
        try {
          ...
          const krakenClient = new Kraken(options);
          return await krakenClient.depositStatus({ asset: base }).then(async (response) => {
            console.log("kraken depositStatus -==>", response, 1527)
            for (let deposit of response) {
      ...
      case 'Bitmart':
        try {
          ...
          console.log('780 bitmart deposit history error==>', error, new Date().toLocaleString());
        }
        break;
    }
  },
